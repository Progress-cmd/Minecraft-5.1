...
Polices::Polices() : textShader("polices.vert", "polices.frag"), textVBO(sizeof(float) * 6 * 4)
{
    ...
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1); // D�sactive la restriction d'alignement d'octets

    for (unsigned char c = 0; c < 128; c++) {
        if (FT_Load_Char(face, c, FT_LOAD_RENDER))
            continue; // erreur -> on skip

        // G�n�ration de la texture
        ...
        // Configuration de la texture (Clamp to edge est crucial pour �viter les artefacts)
